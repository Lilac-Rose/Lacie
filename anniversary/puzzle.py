...
DAILY_ROLE_TEMPLATE = "Day {day} Solver (5th Anni)"
PUZZLE_HUNTER_ROLE_NAME = "Puzzle Hunter (5th Anni)"
...
def _event_day(cfg: dict) -> int | None:
    ...
    if start <= now <= end:
        day = int((now - start).total_seconds() // 86400) + 1
        return day if day <= 7 else None
    return None
...
class PuzzleHunt(commands.Cog):
    ...
    async def _ensure_roles(self):
        """Create all event roles if missing, rename if stale, and persist IDs to puzzle_roles."""
        ...
        for day in range(1, 8):
            ...
            row = c.fetchone()
            role = await self._get_or_create_role(guild, name, existing_id=row[0] if row else None)
            if role:
                c.execute(
        ...
        c.execute("SELECT role_id FROM puzzle_roles WHERE day=0")
        row = c.fetchone()
        hunter_role = await self._get_or_create_role(guild, PUZZLE_HUNTER_ROLE_NAME, existing_id=row[0] if row else None)
        if hunter_role:
            c.execute(
                "INSERT OR REPLACE INTO puzzle_roles (day, role_id) VALUES (?, ?)",
                (0, hunter_role.id),
    ...
    async def _get_or_create_role(
        self, guild: discord.Guild, name: str, existing_id: int | None = None
    ) -> discord.Role | None:
        # Prefer lookup by stored ID so we can rename stale roles
        ...
        if role is not None:
            if role.name != name:
                try:
                    await role.edit(name=name, reason="5th Anniversary role rename")
                    logger.info(f"PuzzleHunt: renamed role to '{name}'")
                except Exception as e:
                    logger.error(f"PuzzleHunt: failed to rename role '{role.name}': {e}")
            return role
        try:
            role = await guild.create_role(name=name, reason="5th Anniversary Event")
            logger.info(f"PuzzleHunt: created role '{name}'")
        except discord.Forbidden:
            logger.error(f"PuzzleHunt: missing Manage Roles permission to create '{name}'")
            return None
        except Exception as e:
    ...
    async def _post_daily_puzzle(self, day: int):
        ...
        full_url = f"https://anniversary.lilacrose.dev{page_url}"
    ...
    async def puzzle_today(self, interaction: discord.Interaction):
        ...
        interaction.client.dispatch("leagues_event", interaction.user.id, "puzzle_today", "done", interaction.user)
    ...
    async def puzzle_leaderboard(self, interaction: discord.Interaction):
        ...
        if _event_day(self.cfg) is None:
            await interaction.response.send_message(
                "The Puzzle Hunt isn't active right now (Aug 16-22, 2026).", ephemeral=True
            )
            return
        ...
        for i, (user_id, count) in enumerate(rows):
            ...
            name = discord.utils.escape_markdown(user.display_name if user else f"User {user_id}")
