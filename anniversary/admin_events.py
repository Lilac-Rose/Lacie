...
LEAGUES_DB  = Path(__file__).parent.parent / "data" / "leagues.db"
SHOWCASE_DIR = (
    Path(__file__).parent.parent.parent
    / "lilacrose.dev" / "bots" / "lacie" / "static" / "uploads" / "showcase"
)
...
class AdminEvents(commands.Cog):
    ...
    showcase_group = app_commands.Group(
        name="showcase",
        description="Showcase admin commands",
        parent=admin_group,
    )
    ...
    @showcase_group.command(
        name="recover",
        description="Re-download showcase images from Discord channel using stored message IDs",
    )
    async def showcase_recover(self, interaction: discord.Interaction):
        if not _check_owner(interaction):
            await interaction.response.send_message("No.", ephemeral=True)
            return
        await interaction.response.defer(ephemeral=True)
        ...
        cfg = _load_config()
        channel_name = cfg.get("showcase_channel_name", "art-showcase")
        guild = self.bot.get_guild(GUILD_ID)
        channel = discord.utils.get(guild.text_channels, name=channel_name) if guild else None
        if channel is None:
            await interaction.followup.send("Could not find the showcase channel.", ephemeral=True)
            return

        SHOWCASE_DIR.mkdir(parents=True, exist_ok=True)

        conn = sqlite3.connect(LEAGUES_DB)
        conn.row_factory = sqlite3.Row
        c = conn.cursor()
        c.execute(
            "SELECT id, message_id FROM showcase_submissions"
            " WHERE message_id IS NOT NULL AND (local_filename IS NULL OR local_filename = '')"
        )
        rows = c.fetchall()
        conn.close()

        if not rows:
            await interaction.followup.send("Nothing to recover — all submissions already have local files.", ephemeral=True)
            return
        ...
        async with aiohttp.ClientSession() as session:
            for row in rows:
                ...
                try:
                    ...
                    logger.warning(f"showcase recover: message {message_id} not found")
                    fail += 1
                    continue
                except Exception as e:
                    logger.warning(f"showcase recover: fetch message {message_id} error: {e}")
                    fail += 1
                    continue

                proxy_url = None
                if msg.embeds and msg.embeds[0].image:
                    proxy_url = msg.embeds[0].image.proxy_url or msg.embeds[0].image.url
                if not proxy_url:
                    logger.warning(f"showcase recover: no image on message {message_id}")
                    fail += 1
                    continue
                ...
                try:
                    async with session.get(proxy_url, timeout=aiohttp.ClientTimeout(total=15)) as resp:
                        if resp.status != 200:
                            logger.warning(f"showcase recover: proxy_url HTTP {resp.status} for message {message_id}")
                            fail += 1
                            continue
                    ...
                    (SHOWCASE_DIR / local_filename).write_bytes(data)

                    conn2 = sqlite3.connect(LEAGUES_DB)
                    c2 = conn2.cursor()
                    c2.execute(
                        "UPDATE showcase_submissions SET local_filename=? WHERE id=?",
                        (local_filename, sub_id),
                    )
                    conn2.commit()
                    conn2.close()
                    ok += 1
                    logger.info(f"showcase recover: saved submission {sub_id} → {local_filename}")
                except Exception as e:
                    logger.warning(f"showcase recover: download error for message {message_id}: {e}")
                    fail += 1

        await interaction.followup.send(
            f"Showcase recovery done: **{ok}** recovered, **{fail}** failed.",
            ephemeral=True,
        )
